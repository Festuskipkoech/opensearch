...
func TestSearchWithInvalidAgentIntentReturns400(t *testing.T) {
	...
	if resp.StatusCode != http.StatusBadRequest {
		t.Errorf("expected 400 for invalid intent, got %d", resp.StatusCode)
	}
}
...
func TestResearchQueryReturnsContent(t *testing.T) {
	resp := doSearch(t, searchRequest{
		Query: "how does the Go garbage collector work",
		Intent: "research",
		MaxResults: 3,
	})
	...
	if !hasContent {
		t.Error("expected at least one result with content populated for research query")
	}
}